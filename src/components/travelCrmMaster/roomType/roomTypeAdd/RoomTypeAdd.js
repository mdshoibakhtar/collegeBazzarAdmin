...
import { addTRCRM_room_type_master, getByTRCRM_room_type_master, updateTRCRM_room_type_master } from "../../../../api/login/Login";
...
const RoomTypeAdd = () => {
    ...
    const submitData = async () => {
        ...
        if (!params?.id) {
            try {
                ...
                const res = await addTRCRM_room_type_master(clone)
                if (res?.error == false) {
                    toastSuccessMessage()
                    setTimeout(() => {
                        navigate(`/room-type-master`)
                    }, 2000)
                } else {
                    alert(res?.message)
                }
            ...
            try {
                const res = await updateTRCRM_room_type_master(params.id, clone)
                if (res?.error == false) {
                    toastSuccessMessage()
                    setTimeout(() => {
                        navigate(`/room-type-master`)
    ...
    useEffect(() => {
        const detbyIdData = async () => {
            try {
                const res = await getByTRCRM_room_type_master(params?.id)
