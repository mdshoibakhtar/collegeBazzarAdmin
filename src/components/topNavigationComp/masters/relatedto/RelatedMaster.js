...
import {  deleteTaskRelatedToById, getTaskRelatedTo } from '../../../../api/login/Login'
import { message } from 'antd'
import MasterList from './masterList/MasterList'
function RelatedMaster() {
    ...
    const getFloorMasters = async (page) => {
        setLoading(true)
        try {
            const res = await getTaskRelatedTo(page, count)
    ...
    const deleteBlockAdd = async (id) => {
        setLoading(true)
        try {
            await deleteTaskRelatedToById(id)
