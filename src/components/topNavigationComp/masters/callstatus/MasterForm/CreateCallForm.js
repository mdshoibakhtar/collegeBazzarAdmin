...
import {  addTypes_Of_ApprovedBy_Master,getApprovedBy_MasterId, getCallByStatusId, postCallStatus, updateCallStatusById, updateTypes_Of_ApprovedBy_Master,} from '../../../../../api/login/Login';
...
() {
    ...
    const submitForm = async (values) => {
        try {
            if (!params?.id) {
                try {
                    const res = await postCallStatus(values);
                    if (res?.statusCode == "200") {
                        toastSuccessMessage(" call-status Added Successfully");
                        navigate(`/call-status-masters`)
                    }
                    blankBtn()
                } catch (error) {
                    alert(error.message)
                ...
                try {
                    const res = await updateCallStatusById(params.id, values);

                    if (res?.statusCode == "200") {
                        toastSuccessMessage("call-status Update Successfully");
                        blankBtn()
                        navigate(`/call-status-masters`)
                    }
    ...
    useEffect(() => {
        const fetchCurrency = async () => {
            try {
                if (params?.id) {
                    const response = await getCallByStatusId(params.id);
