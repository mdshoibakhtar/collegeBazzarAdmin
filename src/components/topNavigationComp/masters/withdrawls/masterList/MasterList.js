...
function MasterList({ data, totalCount, page, count, onChangeVal, confirm, cancel, title ,getFloorMasters}) {
    ...
    return (
        <>
            <div>
                {modalShow && <StatusModel getFloorMasters={getFloorMasters} datas={datas} show={modalShow} onHide={() => setModalShow(false)} />}
                                        ...
                                        <tbody>
                                            {data?.map((item, i) => {
                                                return <tr role="row" className="odd">
                                                    ...
                                                    <td>
                                                        {item?.user_id?.main_wallet}
                                                    </td>

                                                    <td>
                                                        <span className="badge badge-success text-light border-0" style={{ backgroundColor: `${item?.is_active === true ? 'blue' : '#bc3922ab'}`, fontSize: `${item?.status === false ? '0.8rem' : ''}` }}>{item?.status}</span>
                                                    </td>
                                                    <td>
                                                        <div className="d-flex">
                                                            <Link to={`#`} onClick={() =>setDatas (item)} className="btn btn-primary shadow btn-xs sharp me-1"><i className="fa fa-pencil" /></Link>
                                                            {/* <Popconfirm
