...
import {getCreProperty, paginationApiMaster, propertyDelte,} from '../../api/login/Login'
...
function PropertyList() {
    ...
    const getPaginationApi = async (page) => {
        // setLoading(true)
        try {
            const res = await getCreProperty(1 , 100)
                        ...
                        <div className="card-body p-0">
                            <div className="table-responsive active-projects style-1">
                                <div className="tbl-caption">
                                    <h4 className="heading mb-0">Property</h4>
                                    <div>
                                        <Link className="btn btn-primary btn-sm" to="/newproperty" role="button" aria-controls="offcanvasExample">+ New Property</Link>
                                ...
                                <div id="empoloyees-tblwrapper_wrapper" className="dataTables_wrapper no-footer"><div className="dt-buttons"><button className="dt-button buttons-excel buttons-html5 btn btn-sm border-0" tabIndex={0} aria-controls="empoloyees-tblwrapper" type="button"><span><i className="fa-solid fa-file-excel" /> Export Report</span></button> </div><table id="empoloyees-tblwrapper" className="table dataTable no-footer" role="grid" aria-describedby="empoloyees-tblwrapper_info">
                                    <thead>
                                        <tr role="row">
                                            ...
                                            Construction start Date</th>
                                            <th className="sorting" tabIndex={0} aria-controls="empoloyees-tblwrapper" rowSpan={1} colSpan={1} aria-label="Contact Number: activate to sort column ascending" style={{ width: '161.675px' }}>
                                            Construction End Date</th>
                                    ...
                                    <tbody>
                                        {data && data?.map((item) => {
                                            return <tr role="row" className="odd" key={item?._id}>
                                                ...
                                                <td>
                                                    {item?.docuconstruction_end_date}
                                                </td>
                                                <td>
                                                    {item?.layout_image}
                                                </td>
                                                ...
                                                <td>
                                                    <div className="d-flex">
                                                        <Link to={`/newproperty/${item?._id}`} className="btn btn-primary shadow btn-xs sharp me-1"><i className="fa fa-pencil" /></Link>
                                                        <Popconfirm
